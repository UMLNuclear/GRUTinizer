...
// Initialize the GRETINA mapping
void InitMap() {
    HoleQMap[Q1] = 1;
    HoleQMap[Q2] = 2;
    HoleQMap[Q3] = 3;
...
// Handle the UML stack detector data
void HandleUML(TRuntimeObjects& obj) {
  
    // Create a UML detector object to hold data
    TUML *uml = obj.GetDetector<TUML>();
  
    // First, fill summary spectrum of all channels.
    ...
    for(unsigned int i=0; i<uml->fSssd.size(); i++) {
        ...
        if(hit.Charge()>10) obj.FillHistogram("strips",20,0,20,hit.GetChannel()-15); // 1 -16?
        
        // Fill calibrated strip histo
        obj.FillHistogram("strips_aligned",6400,0,64000,hit.GetEnergy(),
                          20,0,20,hit.GetChannel()-15); // 1 -16?
        // Fill uncalibrated strip histo
    ...
    // BEGIN FILLING ROOT FILE DIRECTORIES
    std::string histname = "";
    std::string dirname  = "";
  ...
  if(e1_e2 && e1_e3
     && e1_e2->IsInside(uml->GetPin1().GetEnergy(),uml->GetPin2().GetEnergy())
     && HeLike
     && HeLike->IsInside(uml->ZmQ(),uml->GetZ())
...
void HandleGretina(TRuntimeObjects& obj) {
    ...
    for(unsigned int i=0; i<gretina->Size(); i++) {
        ...
        for(unsigned int j=i+1; j<gretina->Size(); j++) {
            ...
            obj.FillHistogram(dirname,"gg_time",1000,-5000,5000,dtime,
                              1000,0,4000,eng);
...
void MakeHistograms(TRuntimeObjects& obj) {
    ...
    // Get the pointer to the object list
    ...
    // Setup and gates that are given
    SetGates(obj);
    
    // If the event contained detector data, then handle it.
    if(uml) {
