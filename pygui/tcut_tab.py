...
class TCutTab(object):
  ...
  def AddDirectory(self, tdir):
    ...
    for branch in gchain.GetListOfBranches():
      ...
      if cls_name not in self.detector_classes:
        top = self.tree.insert('','end',cls_name, text=cls_name)
        self.tree.insert(top,'end','{}/Draw PID'.format(cls_name),text='Draw PID')
  ...
  def AddCut(self, cut, det_type = None):
    ...
    self.tree.insert(parent, 'end', full_name, text=name, values='2D Cut',image = self.main.icons['tcutg'])
